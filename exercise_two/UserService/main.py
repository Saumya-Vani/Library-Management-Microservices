...
# Borrow a book request and publish it
@app.route('/users/borrow/request', methods=['POST'])
def borrow_book():
    data = request.get_json()
    required_fields = {'student_id', 'book_id', 'date_borrowed', 'date_returned'}
    ...
    # Query the number of books currently borrowed by the student
    borrowed_count = get_borrowed_count(student_id)
    if borrowed_count is None:
        return jsonify({"error": "Unable to check borrowing limit. Please try again later."}), 500

    # Check if borrowing limit is exceeded
    if borrowed_count >= 5:
        return jsonify(
            {"error": f"Borrow limit exceeded: Student {student_id} has already borrowed {borrowed_count} books."}), 403
    ...
    try:
        ...
        return jsonify({"error": f"Error connecting to RabbitMQ: {str(e)}"}), 500

# Helper function to get the borrowed count from BorrowService
def get_borrowed_count(student_id):
    try:
        response = requests.get(f"http://borrow_service:5003/test_borrow_count/{student_id}")
        if response.status_code == 200:
            return response.json().get('borrowed_count', 0)
        else:
            return None
    except requests.RequestException as e:
        print(f"Error querying BorrowService: {e}")
        return None
